...
## Follow-up — 2026-09-07T11:41:33Z
...
Execute Phase 1 and Phase 2 of the `ARCHITECTURE_AUDIT_REPORT.md` to completely overhaul the state management architecture of UWGAS. You will migrate the monolithic `App.tsx` to the newly established Zustand stores and enforce strict architectural boundaries around the pure math engine. 

Use a very large team of agents to ensure this massive refactor is completed concurrently and efficiently.
...
### R1. Dismantle the God Component
Remove all domain state (`useState`) from `App.tsx`. Migrate all state logic, JSON import/export merging, and persistence handlers into the newly created Zustand stores (`src/state/store.ts` and `src/state/uiStore.ts`). `App.tsx` must be reduced to a purely structural layout shell handling only top-level routing and UI scaffolding.

### R2. Eradicate Prop Drilling (Plug and Play)
Refactor all UI components (e.g., `GlobalSetupCard.tsx`, `ProgressionView.tsx`, `SettingsRootView.tsx`) to consume their required state directly from the Zustand stores. You MUST use selector hygiene (e.g., pulling individual properties) to prevent re-render storms. Remove all prop-drilling related to global state.

### R3. Enforce Math Engine Isolation (The Vault)
Implement the 2-Tier Math Isolation strategy outlined in the audit report. Ensure that `src/math/tormek.ts` (Tier 1) remains a pure algorithmic module with zero React dependencies or UI-specific types. Establish the Tier 2 adapter service to bridge the Zustand store with the pure math engine.

### R4. Storage Modernization Integration
Wire the application to correctly utilize the new debounced, Zod-validated `uwgas_app_state_v1` local storage mechanism established in `src/state/store.ts`. Ensure legacy fallback migrations handle the transition gracefully.
