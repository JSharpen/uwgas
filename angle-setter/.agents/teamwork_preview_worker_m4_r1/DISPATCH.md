## 2026-09-07T18:57:00Z

<USER_REQUEST>
You are Replacement Worker M4_r1 (UI Component Refactoring & App.tsx Shell Decomposition).
Identity: teamwork_preview_worker_m4_r1
...
State Architecture: /home/jordancarruthers/Documents/GitHub/uwgas/angle-setter/.agents/teamwork_preview_worker_m3/handoff.md
Math Architecture: /home/jordancarruthers/Documents/GitHub/uwgas/angle-setter/.agents/teamwork_preview_worker_m2/handoff.md
...
1. Extract Views:
   ...
   - Create `src/views/SettingsView.tsx`: Sub-router reading `settingsView` from `useUIStore`, rendering `SettingsRootView`, `MeasurementSettingsView`, `HardwareManagerView`, `MachineManagerView`, `ImportExportPanel`, or `GlossaryPage`. Provide a back button to return to 'root'.
2. Eradicate Prop Drilling with Atomic Selector Hygiene across all UI components:
   ...
   - `ProgressionView`: 0 props. Subscribes to `useWheelResults()` from `src/services/calculationService` and `useProgressionState()` from `src/state/store`. In `StepCard`, eliminate drilled props and subscribe directly to `useStore` / `useUIStore` actions.
   ...
   - `SettingsRootView`: 0 props. Connect to `useUIStore(s => s.setSettingsView)`.
