...
| **JOB-011** | Device, Screen Size & Density Optimization | `[IN PROGRESS]` | **MEDIUM** | Define and enforce mobile viewport targets: 360px strict minimum (no overlap/wrapping) and 390px comfortable baseline. Audit existing UI components against these constraints. |
...
| **JOB-024** | Global State & Architecture Overhaul (Zustand/Zod) | `[IN PROGRESS]` | **HIGH** | 4-tier architectural restructure. Migrate from monolithic App.tsx state to slice-based Zustand v5 store. Modernize local storage with 300ms debounce and Zod schema validation. Implement two-tier strict isolation for the pure math engine. |
